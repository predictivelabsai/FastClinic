...
Address lookup uses a configurable Nominatim-compatible `MARKET_GEOCODER_URL`, with a configurable Photon-compatible `MARKET_FALLBACK_GEOCODER_URL` for exact Baltic addresses missing from the primary index. Both defaults use OpenStreetMap data. The implementation caches results, runs server-side only, identifies FastClinic in its User-Agent, and uses a shared single-threaded database gate with at least 1.1 seconds between requests. `MARKET_GEOCODER_MIN_INTERVAL_SECONDS` can slow an approved/self-hosted endpoint further but cannot make the public defaults exceed one request per second. Newly evidenced addresses are mapped during the same collection task, and every worker tick drains any addresses left pending by a transport interruption. No autocomplete or browser-triggered geocoding is offered. Disable either endpoint with an empty URL or use a self-hosted/approved provider for larger recurring imports. Country, city, street, and house number are validated before any pin is saved; ambiguous/empty matches are cached for review. Reusing an unchanged address does not repeat a successful lookup.
